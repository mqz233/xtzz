...
class LSTM_FEATURES(object):
    ...
    def split_sequence(self, sequence, n_steps):#滑动窗口大小是最后一帧数：1000
        X, y = np.array([]), list()
        for i in range(len(sequence)):
            num_zeros = n_steps - i-1
            x_size = array(sequence[0]).shape[0] #随意取一个，得到input_size=210/110
    ...
    def lstm(self):
        ...
        data = data[:self.frame+1] #起码两个初始data值，才能构成1个x，1个y，所以是frame+1 (frame从1开始取值)
        # dataY = datas['data_y']
        ...
        class RNN(nn.Module):
            def __init__(self, input_size):
                ...
                self.lstm = nn.LSTM(input_size, 100, 2)  # 输入数据2个特征维度，6个隐藏层维度，2个LSTM串联，第二个LSTM接收第一个的计算结果
                self.out = nn.Linear(100, input_size)  # 线性拟合，接收数据的维度为6，输出数据的维度为1
        ...
        rnn.load_state_dict(torch.load("E:\OneDrive - mail.dlut.edu.cn\桌面\djangoProject2\PredictionModel\steps1000_LSTM_features.pth"))  # 加载模型
        rnn.cuda(0)
...
# result = LSTM_FEATURES( "E:\项目数据集\dataset\json_output_5055555_dynamic.json",1).lstm()
