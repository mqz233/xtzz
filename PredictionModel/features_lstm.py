...
class LSTM_FEATURES(object):
    ...
    def lstm(self):
        ...
        torch.save(rnn.state_dict(), "LSTM_features.pth")#保存模型

        # rnn.load_state_dict(torch.load("LSTM_features.pth"))  # 加载模型
        ...
        yhat_list = []
        for i in range(1200 - (num_data + self.n_steps)):
            pred = rnn(var_dataX)
            # pred_test = pred.view(-1).data.numpy()  # 转换成一维的ndarray数据，这是预测值 ,dataY为真实值
            pred_test = pred.view(-1, input_size).data.numpy()
...
# result = LSTM_FEATURES(2, "..\data\json_output_5055555_dynamic.json").lstm()
