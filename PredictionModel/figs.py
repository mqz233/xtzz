...
# from tensorflow_features_lstm import Lstm
from PredictionModel.figs_features import LSTM_FEATURES
# from PredictionModel.figs_features_steps1000 import LSTM_FEATURES
import os
...
class LSTM_PREDICT(object):
    ...
    def features_predict(self):
        # features_pre = LSTM_FEATURES(self.n_steps, self.datas_path, self.frame).lstm()  # 3:步长——>预测dataX本身时的步长可以随意取
        features_pre = LSTM_FEATURES(self.datas_path,self.frame).lstm()
        return features_pre
    ...
    def lstm(self):
        ...
        # # 训练集划分
        # train_size = self.frame  # 已有的数据全用来训练模型
        ...
        #     var_y = Variable(y_train).type(torch.FloatTensor)  # Tensor：3
        ...
        rnn.load_state_dict(torch.load("PredictionModel\LSTM_svv.pth"))  # 加载模型
        rnn.cuda(0)
        # 四、模型测试
        # 准备测试数据
        # dataX = np.array(dataX)
        dataX = np.array(dataX[:self.frame])
        # 添加预测的pre_dataX
        ...
        pred_test = torch.Tensor.cpu(pred.view(-1, 10).data).numpy()  # 变为-1*10的矩阵，再求和第二维，实现每隔10个标签求和
        sum_y = dataY.sum(axis=1)
        sum_pred = pred_test.sum(axis=1)
        return sum_pred[self.frame], sum_pred[-1],sum_pred  # 获得frame下一帧、最后1000帧的预测值
...
def pre_result(path,num_frame):
    # result = LSTM_PREDICT("..\data\json_output_5055555_static.json").lstm()
    pre_next_array = np.array([])
    pre_result_array = np.array([])
    # sum_pred_list=[]

    #num_frame: 模拟文件夹下有多少帧数据,todo:实时获取该文件夹下多少条数据
    for i in range(1, num_frame):
        # steps = 3
        # pre_next, pre_result,pre_all = LSTM_PREDICT(3, "E:\项目数据集\dataset\json_output_5055555_dynamic.json", i).lstm()
        pre_next, pre_result, pre_all= LSTM_PREDICT(3, path, i).lstm()
        pre_next_array = np.append(pre_next_array, pre_next)
        pre_result_array = np.append(pre_result_array, pre_result)
        # sum_pred_list.append(pre_all)
    # datas = jo().convertJsonToDict("E:\项目数据集\dataset\json_output_5055555_dynamic.json")
    ...
    if type:
        ...
        plt.title('静态ZZ数据预测结果')
    plt.plot(pre_next_array, 'r', label='next_prediction')
    # plt.plot(np.append(np.zeros(num_frame),pre_all[num_frame+1:]),'r')#todo：整体坐标向右移动num_frame,不能添0
    plt.plot(pre_result_array, 'g', label='result_prediction')
    plt.plot(sum_y, 'b', label='real')
    plt.legend(loc='best')
    # plt.savefig(os.path.join(rc().getImagePath(), self.file_name + r'.png'))
    plt.show()
    return list(sum_y), list(pre_next_array), list(pre_result_array)
# pre_result("E:\项目数据集\dataset\json_output_5055555_dynamic.json",200)
